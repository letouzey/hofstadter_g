...
Proof.
  change (apply (ksubstk k) u) with (napply (ksubstk k) 1 u).
  rewrite napply_ksubstk. apply PrefixSeq_napply.
  apply ksubst_noerase.
  apply ksubst_prolong.
Qed.

Lemma round_by_incr_function f :
  (forall p, f p < f (S p)) ->
...
Lemma roundbis_by_incr_function f :
  (forall p, f p < f (S p)) ->
  forall n, f 0 < n -> exists p, f p < n <= f (S p).
Proof.
 intros Hf n H0.
 destruct n. lia.
 destruct (round_by_incr_function f Hf n) as (p,Hp); try lia.
 exists p. lia.
Qed.

Definition roundk k p := length (apply (ksubstk k) (take p (kseq k))).

Lemma roundk_cumul k n :
  roundk k n = n + cumul (kseq k) n.
Proof.
 unfold roundk.
 induction n. trivial.
...
Lemma roundk_S k p : roundk k (S p) = roundk k p + S (kseq k p).
Proof.
 unfold roundk.
...
Lemma roundk_round k n :
  0 < n ->
  exists p, roundk k p < n <= roundk k (S p).
Proof.
 intros Hn.
 apply roundbis_by_incr_function.
 - intros p. rewrite roundk_S. lia.
 - apply Hn.
Qed.
...
Lemma countk_roundk k p :
 count (kseq k) k (roundk k p) = p.
Proof.
 rewrite count_nbocc.
 set (u := take p (kseq k)).
 ...
 fold (roundk k p) in Hu. unfold letter in Hu. rewrite <- Hu.
 rewrite nbocc_ksubstk. unfold u. apply take_length.
 apply take_kseq_letters.
Qed.

Lemma kseq_roundk k p : kseq k (roundk k p) = k.
Proof.
 rewrite <- (take_nth (kseq k) (roundk k (S p)) _ k).
 2:{ rewrite roundk_S; lia. }
 set (u := take (S p) (kseq k)).
 assert (Hu : PrefixSeq u (kseq k)).
 { red. unfold u. f_equal. now rewrite take_length. }
 apply ksubstk_prefix in Hu. red in Hu. unfold u at 2 in Hu.
 fold (roundk k (S p)) in Hu. rewrite <- Hu.
 clear Hu. unfold u. rewrite take_S, apply_app.
 rewrite app_nth2 by (unfold roundk; lia).
 replace (_-_) with 0 by (unfold roundk; lia). simpl.
 now rewrite app_nil_r, ksubstk_alt by apply kseq_letters.
Qed.

Lemma countk_S_roundk k p :
 count (kseq k) k (S (roundk k p)) = S p.
Proof.
 rewrite count_nbocc, take_S, nbocc_app, <- count_nbocc, countk_roundk.
 rewrite kseq_roundk. simpl. rewrite Nat.eqb_refl. lia.
Qed.
...
Lemma roundk_round' k n p :
  0 < n ->
  p = count (kseq k) k n ->
  roundk k (p-1) < n <= roundk k p.
Proof.
  intros Hn Hp.
  destruct (roundk_round k n Hn) as (p' & (U,V)).
  assert (U' := count_mono (kseq k) k _ _ U).
  rewrite countk_S_roundk, <- Hp in U'.
  assert (V' := count_mono (kseq k) k _ _ V).
  rewrite countk_roundk, <- Hp in V'.
...
Lemma cumul_kseq_grow k n : 0<n -> cumul (kseq k) n < cumul (kseq (S k)) n.
Proof.
 ...
 - destruct (Nat.le_gt_cases k (kseq (S k) n)) as [LE|LT].
   ...
   + assert (Hk : S k <> 0) by lia.
     ...
     { rewrite Nat.le_ngt. intro.
       assert (E1 : m <= roundk k (a k m)) by (apply roundk_round'; trivial).
       assert (E2 := roundk_cumul k (a k m)).
       ...
       assert (E5 := roundk_cumul (S k) (a (S k) m -1)).
       assert (E6 : roundk (S k) (a (S k) m - 1) < m).
       { apply roundk_round'; trivial. }
       lia.
     }
