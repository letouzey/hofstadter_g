...
Technical report version 1.3, Sept 2015, \href{http://creativecommons.org/licenses/by/4.0/}{license CC-BY}
}
...
\begin{itemize}
\item Case $k=1$. Then $n-1 = \fibrest$. A first induction
  hypothesis gives $G(n-1) = \Sigma F_{i-1}$. Since the initial
  decomposition was relaxed, $\fibrest$ cannot contain $F_1$,
...
$n=F_1+\fibrest$, hence
$n+1=F_2+\fibrest$ is a relaxed decomposition. By the previous
...
If $F_1$ isn't part of this decomposition, then $n+1=F_1+\fibrest$
...
$flip(n)$ and the leftmost node $1+F_{k}$ will be equal to
the distance between $n$ and the rightmost node $F_{k+1}$,
hence:
...
Moreover $1+F_k+n \ge 1+F_k \ge 2$, so
...
and $F_{k+1}$ are at a distance of 0 or 1. But we know that
...
When $n\le 3$, $n$ is never \mbox{2-even}, and we indeed have
...
\item Case $n$ 2-odd. As in all the remaining cases, we're now trying here
